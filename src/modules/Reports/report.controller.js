...
export const getAllOrdersFilterNew = catchError(
   async(req , res , next)=>{
      ...
      if(start && end){
         ...
         dateObj = {$gte: new Date(`${specificYear}-01-01T00:00:00z`).getTime() , $lte: new Date(`${specificYear}-12-31T24:00:00z`).getTime()}
      }else{
         dateObj = {$gte: new Date(`${currentYear}-${currentMonth}-01T00:00:00z`).getTime()}
      }
...
// //& Get All Orders By Date From Specific Date To Specific Date By Patient Name  :
// export const getOrderByPatientName = catchError(
//    async(req , res , next)=>{
//       const {start ,  end , patient , company_id} = req.query ;
//       let orders = [];
